...
class User(Base):
    ...
    tracker_diagrams: Mapped[list["TrackerDiagram"]] = relationship("TrackerDiagram", back_populates="users")
...
class WorkoutTracker(Base):
    ...
    users: Mapped[User] = relationship("User", back_populates="workout_trackes")
    users_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"), nullable=True)
...
class TrackerDiagram(Base):
    ...
    sleep_trackers: Mapped[list["SleepTracker"]] = relationship("SleepTracker", secondary="diagram_bridges", back_populates="tracku")
